...
from lstore.lock_manager import LockConflict  # for handling no-wait lock conflicts
...
class Query:
    ...
    # ---- Lightweight merge: piggyback on write ops ----

    def _maybe_apply_one_merge(self) -> None:
           """On write paths, check if there's a merge result ready and apply one."""
           try:
               if not hasattr(self.table, "_merge_lock") or not hasattr(self.table, "_merge_results"):
                   return
